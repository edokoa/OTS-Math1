...
void draw(){
  ...
  if(keyPressed){
    ...
    if (keyCode == UP){                                    //If we press up, we draw the fire and advance the ship
      ...
      float vert1X = shipX + 20*cos(radians((360/12)*5)+radians(rotation));      
      float vert1Y = shipY + 20*sin(radians((360/12)*5)+radians(rotation));    
    
      float vert2X = shipX + 40*cos(radians((360/12)*6)+radians(rotation));      
      float vert2Y = shipY + 40*sin(radians((360/12)*6)+radians(rotation));    //Tip of the fire
    
      float vert3X = shipX + 20*cos(radians((360/12)*7)+radians(rotation));      
      float vert3Y = shipY + 20*sin(radians((360/12)*7)+radians(rotation));    
...
void advanceShip(){
  shipX = shipX + 10*cos(radians(rotation));
  shipY = shipY + 10*sin(radians(rotation)); 
}
